...
void WADReader::ReadBlockMapHeader(const uint8_t* pWADData, int offset, BlockMapHeader& blockmapheader)
{
    blockmapheader.nmColum = Read2Bytes(pWADData,offset);
    blockmapheader.nmLine = Read2Bytes(pWADData,offset + 2);
    blockmapheader.originX = Read2Bytes(pWADData,offset + 4);
    blockmapheader.originY = Read2Bytes(pWADData,offset + 6);
}

void WADReader::ReadBlockMapOffset(const uint8_t* pWADData, int offset, BlockMapOffset& blockmapoffset ,int nOffset)
{
    blockmapoffset.offset0 = Read2Bytes(pWADData, offset);
    blockmapoffset.offset1 = Read2Bytes(pWADData, offset + 2);
    blockmapoffset.offsetn = Read2Bytes(pWADData, nOffset + offset);
}
